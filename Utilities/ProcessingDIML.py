...
class ProcessingDIML:
    ...
    @staticmethod
    def _LoadPaths():
        return ProcessingDIML._GetPairs(ProcessingDIML.BASE)
    ...
    def ProcessBatches(pairs: list, prefix: str, save_path: str, batch_size: int):
        ...
        N = len(pairs)
        imagesT_mm = open_memmap(save_path + prefix + '_images_split.npy', mode='w+', dtype=np.uint8, shape=(N, 3, 756, 1344))
        imagesN_mm = open_memmap(save_path + prefix + '_images_norm_split.npy', mode='w+', dtype=np.float32, shape=(N, 3, 756, 1344))
        imagesS_mm = open_memmap(save_path + prefix + '_images_stand_split.npy', mode='w+', dtype=np.float32, shape=(N, 3, 756, 1344))

        depthT_mm  = open_memmap(save_path + prefix + '_depths_split.npy', mode='w+', dtype=np.float32, shape=(N, 756, 1344))
        depthC_mm  = open_memmap(save_path + prefix + '_depths_clipped_split.npy', mode='w+', dtype=np.float32, shape=(N, 756, 1344))
        depthN_mm  = open_memmap(save_path + prefix + '_depths_norm_split.npy', mode='w+', dtype=np.float32, shape=(N, 756, 1344))

        mask_mm    = open_memmap(save_path + prefix + '_mask_split.npy', mode='w+', dtype=bool, shape=(N, 756, 1344))
        ...
        for start in range(0, N, batch_size):
            ...
            imagesT, depth_mapsT = ProcessingDIML._LoadAllImages(paths)
            ...
            imagesN = ProcessingDIML._NormalizeRGB(imagesT)
            imagesS = ProcessingDIML._StandardizeRGB(imagesN)
            imagesN_mm[start:end] = imagesN
            imagesS_mm[start:end] = imagesS

            # 2.4. Generate a mask for depth pixel out of range
            masks = ProcessingDIML._GenerateDepthMaskBatch(depth_mapsT)
            ...
            depth_mapsN, minmax_list = ProcessingDIML._NormalizeDepth(depth_mapsC)
    ...
    def GenerateNPYFiles(batch_size: int = 32):
        ...
        # 3. Process the data
        ProcessingDIML.ProcessBatches(test_pairs, 'test', path, batch_size)